...
void validacao_cruzada(double matriz[tam_conjunto][num_caracteristicas])
{
    long long i, j;
    double VP, VN, FP, FN;
    int *binario, resultado, iteracoes = 0;
    
    VP = VN = FP = FN = 0.0;

    int m[tam_conjunto], k;
    ...
    do
    {
        printf("\rporcentagem: %f\n", (float)((100*iteracoes)/max_iteracoes));
        ...
        {
            if(m[j] == 0) //se o elemento do vetor for 0, a linha da matriz de VCs associada a esse elemento contém um VC de teste.
            {
                resultado = KNN(matriz, m, matriz[j]); //chama a função KNN para clasificar esse VC.
